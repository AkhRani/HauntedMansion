...
func _process(delta):
	...
	if position == target and dir != Vector2():
		...
		# get a "bump" frame
		dt = delta * speed
		position = start.lerp(try_target, dt)
...
func adjust_camera(delta, moving):
	if initial_pan < 2:
		...
		if initial_pan > 2:
			...
			camera.zoom = Vector2(1,1).lerp(Vector2(MAX_SCALE,MAX_SCALE), initial_pan/2)
		if initial_pan >= 1:
			camera.position = Vector2(0,0)
		else:
			camera.position = to_local(SCREEN_CENTER).lerp(Vector2(0,0), initial_pan)
		return
	if moving:
